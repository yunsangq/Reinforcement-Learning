...
while True:
    if idx == 0:
        save_world.append(world)
        newlist = copy.deepcopy(action_prob)
        save_action_prob.append(newlist)
        save_idx.append(idx)
    ...
    for i, j in states:
        for action in actions:
            ...
            newWorld[i, j] += 0.25 * (REWARD + world[newPosition[0], newPosition[1]])

    if np.sum(np.abs(world - newWorld)) < 1e-4:
        print('Random Policy')
        print(newWorld)
        save_world.append(newWorld)
...
w.create_text(130, 20, text='Two array version time: %.3fsec' % duration)
